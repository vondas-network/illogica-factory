...
def generate(data: dict) -> None:
    try:
        ...
        generated_title = data.get('generatedTitle') # Color of subtitle text
        # generated_title = data.get('generatedTitle') # Color of subtitle text
        ...
        try:
            final_video_path = generate_video(combined_video_path, tts_path, subtitles_path, n_threads or 2, subtitles_position, text_color or "#FFFF00", generated_title)
        except Exception as e:
...
# First API route (that triggers another route)
@app.route('/trigger_route', methods=['GET'])
def trigger_route():
    # Sleep time between the two route triggers (e.g., 5 seconds)
    time.sleep(2)

    # Trigger the second API route within the Flask app
    response = requests.get('http://127.0.0.1:8080/target_route')

    # Return a response combining both results
    return jsonify({
        "trigger_response": "First route triggered successfully!",
        "target_response": response.json()
    })

# Second API route
@app.route('/target_route', methods=['GET'])
def target_route():
    ...
    generator = ContentGenerator(cohere_api_key)

    # Generate the initial seed words
    initial_seeds = generator.generate_random_words(25)
    print(f"initial_seeds: {initial_seeds}")
    # Process the prompts, generating results
    results = generator.process_prompts(initial_seeds, iterations=1, num_sentences=3)


    # # Step 1: Directly use the 'results' variable (no need to open a JSON file)
    for result in results:
        generated_title = result['title']
        generated_subject = result['description']
    ...
    json_data['videoSubject'] = generated_subject  # Example of replacing the value

    modified_title = generated_title.replace(':', ' -')
    if "Title - " in modified_title:
        modified_title = modified_title.replace("Title - ", "")

    # Using regex to remove parentheses and the text within them
    # modified_title = re.sub(r'\(.*?\)', '', modified_title).strip()

    json_data['generatedTitle'] = modified_title  # Inject a new title
    print(json_data)

    # # Modify or inject new strings into the dictionary
