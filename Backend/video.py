...
def save_video(video_url: str, directory: str = r"..\temp") -> str:
    """
    ...
    """
    video_id = uuid.uuid4()
    video_path = f"/Users/rr/Desktop/MoneyPrinter-GEN-STABLE-mc/temp/{video_id}.mp4"
    ...
    vidurlheaders = {
        "User-Agent": f"fuckoff-{rand_str(10)}-the_garbage_man"
    }
    print('_~_~_~_~_~_~_~Headers_~_~_~_~_~_~~_')
    print(vidurlheaders)
    response = requests.get(video_url, headers=vidurlheaders, stream=True)
...
def generate_video(combined_video_path: str, tts_path: str, subtitles_path: str, threads: int, subtitles_position: str,  text_color: str, generated_title: str) -> str:
    """
    ...
    print("GENERATED FILE COME ON", generated_title)
    print("WTF")
    print("COMBINED_VIDEO_PATH", combined_video_path)
    ...
    print("FINAL VIDEO NAME ------>", generated_title)

    # without_title_prefix = generated_title.replace('Title - ', '', 1)
    outputPath = f"../temper/{generated_title}.mp4"
    # outputPath = f"../temp/{generated_title}.mp4"

    print(outputPath)

    outputPath = re.sub(r'\(.*?\)', '', outputPath).strip()

    # Get the current timestamp
    # timestamp = time.time()    
    # # print("TIMESTAMP", int(timestamp))
    # # print(f"../temp/output-{int(timestamp)}.mp4")




    # superfile = f"../temp/{generated_title}-{int(timestamp)}.mp4"
    # superfile = f"../temp/{generated_title}.mp4"
    ...
    finaltime = f"{generated_title}.mp4"
